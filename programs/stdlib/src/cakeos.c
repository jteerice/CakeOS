...
struct command_argument* cakeos_parse_command(const char* command, int max) {
	...
	root_command = cakeos_malloc(sizeof(struct command_argument));
	if (!root_command) {
		goto out;
	}
	...
	token = strtok(NULL, " ");
	while (token != 0) {
		struct command_argument* new_command = cakeos_malloc(sizeof(struct command_argument));
		if (!new_command) {
			break;
		}
...
int cakeos_system_run(const char* command) {
	...
	struct command_argument* root_command_argument = cakeos_parse_command(buf, sizeof(buf)); 
	if (!root_command_argument) {
		return -1;
	}

	return cakeos_system(root_command_argument);
}
