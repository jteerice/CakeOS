...
struct filesystem* filesystems[CAKEOS_MAX_FILESYSTEMS];
struct file_descriptor* file_descriptors[CAKEOS_MAX_FILE_DESCRIPTORS];

static struct filesystem** fs_get_free_filesystem() {

	int i = 0;

	for (i = 0; i < CAKEOS_MAX_FILESYSTEMS; i++) {
		if (filesystems[i] == 0) {
...
void fs_insert_filesystem(struct filesystem* filesystem) {
	...
	if (!fs) {
		print("Error inserting filesystem");
		while(1) {}
	}
...
static int file_new_descriptor(struct file_descriptor** desc_out) {
	...
	for (int i = 0; i < CAKEOS_MAX_FILE_DESCRIPTORS; i++) {
...
static struct file_descriptor* file_get_descriptor(int fd) {
	...
	if (fd <= 0 || fd >= CAKEOS_MAX_FILE_DESCRIPTORS) {
		return 0;
	}
...
int fopen(const char* filename, const char* mode_string) {
	...
	// Cannot fopen the root directory
	if (!root_path->first) {
		res = -EINVARG;
	}
