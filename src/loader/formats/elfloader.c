...
static bool elf_is_executable(struct elf_header* header) {
	...
	return header->e_type == ET_EXEC && header->e_entry >= CAKEOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
int elf_validate_loaded(struct elf_header* header) {
	...
	return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header)) ? CAKEOS_ALL_OK : -EINFORMAT;
}
