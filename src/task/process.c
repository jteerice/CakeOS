...
static struct process* processes[CAKEOS_MAX_PROCESSES] = {};
...
struct process* process_get(int process_id) {
	...
	if (process_id < 0 || process_id >= CAKEOS_MAX_PROCESSES) {
		return NULL;
	}
...
static int process_find_free_allocation_index(struct process* process) {
	...
	int res = -ENOMEM;
	for(int i = 0; i < CAKEOS_MAX_PROGRAM_ALLOCATIONS; i++) {
...
// FOR MULTITASKING
// process_map_to()
// Loop though all tasks of the process and paging_map_to()
// So all tasks have access to the program memory
...
void process_switch_to_any() {
	...
	for (int i = 0; i < CAKEOS_MAX_PROCESSES; i++) {
		if (processes[i]) {
	...
	// Change to switch back to shell if no processes
...
int process_map_binary(struct process* process) {
	...
	int res = 0;
	paging_map_to(process->task->page_directory, (void*) CAKEOS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITABLE);
...
int process_map_memory(struct process* process) {
	...
	// Map the stack
	paging_map_to(process->task->page_directory, (void*)CAKEOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack + CAKEOS_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITABLE);
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot) {
	...
	program_stack_ptr = kzalloc(CAKEOS_USER_PROGRAM_STACK_SIZE);
